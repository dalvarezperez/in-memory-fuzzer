...
class debugger():
    def __init__(self):
        ...
        f=open(INTERNAL_ANALYSIS_PUT_PATH,"rt")
        lines=f.readlines()
        f.close()
        scsv = ""
        for l in lines:
          if(".py> " in l):
            scsv += l.split(".py> ")[1]
        self.functions_file_descriptor  = StringIO(scsv)
    ...
    def get_debug_event(self):
        ...
        if kernel32.WaitForDebugEvent(byref(debug_event), INFINITE):
            ...
            # Stop debugging when the target process ends its execution
